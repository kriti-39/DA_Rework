...
const GoldenWaves = () => {
  ...
  useEffect(() => {
    ...
    const NUM   = 38;   // number of wave lines
    const SPEED = 0.005;
    ...
    const draw = () => {
      ...
      const cy = H * 0.52; // vertical center of the ribbon

      for (let i = 0; i < NUM; i++) {
        const norm   = i / (NUM - 1);           // 0 → 1
        const offset = (norm - 0.5) * 2;        // -1 → +1

        // Lines near the center are brightest
...
const Hero2 = () => {
  ...
  // Name moves DOWN, image+subtext move UP together
  ...
  return (
    <section className="relative w-full overflow-x-hidden">
      ...
      <div className="relative flex flex-col items-center">
        ...
        <motion.div
          className="relative z-10 text-center w-full pt-24 md:pt-30 px-4"
          style={{ y: nameY }}
        >
          <motion.h1
            className="font-cinzel font-semibold text-[2rem] sm:text-[2.8rem] md:text-[3.8rem] lg:text-[4.8rem] xl:text-[5.5rem] text-[#f0e6d0] tracking-[0.05em] leading-none"
        ...
        {/* ── PHOTO + SUBTEXT clubbed together — z-20, moves UP as one unit ── */}
        <motion.div
          className="relative z-20 w-full flex flex-col items-center -mt-[18vw]"
          style={{ y: imgY }}
        >
          {/* Photo — with golden waves behind */}
          ...
          {/* Subtext — directly below photo, no gap */}
          <motion.div
